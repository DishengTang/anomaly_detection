...
class DCI(nn.Module):
    ...
    def forward(self, seq1, seq2, adj, msk, samp_bias1, samp_bias2, cluster_info, cluster_num):
        h_1 = self.gin(self.norm_layer(seq1), adj)
        h_2 = self.gin(self.norm_layer(seq2), adj)
...
class DGI(nn.Module):
    ...
    def forward(self, seq1, seq2, adj, msk, samp_bias1, samp_bias2, lbl):
        criterion = nn.BCEWithLogitsLoss()
        h_1 = torch.unsqueeze(self.gin(self.norm_layer(seq1), adj), 0)

        c = self.read(h_1, msk)
        c = self.sigm(c)

        h_2 = torch.unsqueeze(self.gin(self.norm_layer(seq2), adj), 0)
...
class SupCL(nn.Module):
    def __init__(self, dataset, args, device, augmentor):
        ...
        self.device = device
        # self.encoder = GCNConv(dataset.num_features, args.hidden)
        ...
        self.topology_eigv = dataset.data.a.to(self.device)
        ...
        self.feature_trans =  nn.Sequential(nn.BatchNorm1d(dataset.num_features, affine=False),
                    nn.Linear(dataset.num_features, 300),
                    nn.Dropout(p=args.dropout),
                    nn.ELU())
        self.topology_trans =  nn.Sequential(nn.BatchNorm1d(args.max_freqs, affine=False),
                    nn.Linear(args.max_freqs, 300),
                    nn.Dropout(p=args.dropout),
                    nn.ELU())
        self.MLP =  nn.Sequential(
                    ...
                    nn.Dropout(p=args.dropout),
                    nn.ELU())
        self.feature_trans.apply(init_weights)
        self.topology_trans.apply(init_weights)
        self.MLP.apply(init_weights)
        self.gate1 = nn.Linear(args.hidden * 3, args.hidden)
        self.gate2 = nn.Linear(args.hidden * 3, args.hidden)
        self.gate3 = nn.Linear(args.hidden * 3, args.hidden)
    ...
    def forward(self, x, edge_index, y, train_mask):
        ...
        h_2 = self.MLP(self.feature_trans(x))
        h_3 = self.MLP(self.topology_trans(self.topology_eigv))
        concat = torch.cat((h_1, h_2, h_3), 1)
        context_gate = self.sigmoid(self.gate1(concat))
        feature_gate = self.sigmoid(self.gate2(concat))
        topology_gate = self.sigmoid(self.gate3(concat))
        h_1 = h_1 * context_gate
        h_2 = h_2 * feature_gate
        h_3 = h_3 * topology_gate
        h_1 = h_1 + h_2 + h_3
