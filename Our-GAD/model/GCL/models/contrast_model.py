...
from model.GCL.losses import Loss
from model.GCL.models import get_sampler
...
class SupervisedContrast(torch.nn.Module):
    def __init__(self, device, tau, base_tau):
        super(SupervisedContrast, self).__init__()
        self.device = device
        self.tau = tau
        self.base_tau = base_tau

    def forward(self, anchor, contrast, mask):

        torch.cuda.empty_cache()
        # compute logits
        logits = torch.div(torch.matmul(F.normalize(anchor), F.normalize(contrast).T), self.tau)
        ...
        logits_mask = torch.scatter(torch.ones_like(mask), 1, torch.arange(anchor.shape[0]).view(-1,1).to(self.device),0)
        mask = mask * logits_mask

        # compute log_prob
        exp_logits = torch.exp(logits) * logits_mask  #.to('cpu')
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True))  # .to('cpu')

        # compute mean of log-likelihood over positive
        mean_log_prob_pos = (mask * log_prob).sum(1) / mask.sum(1)

        # loss
        loss = - (self.tau / self.base_tau) * mean_log_prob_pos.to(self.device)
        loss = loss.mean()
